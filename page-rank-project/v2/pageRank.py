...
class PageRank(object):
    ...
    def initAllStructs(self):
        ...
        self.inList = [inList[i] for i in self.nodeIDs]

        # 3. build out-degree structure to hold reference to the out degree of all nodes
        self.outDegree = [len(self.adjList[n]) for n in self.nodeIDs]

        # 4. conditionally build list of all sink nodes, for type 3 sink handling only)
        self.sinks = []
        if not self.selfLoops:
            self.sinks = [i for i in self.nodeIDs if self.outDegree[i] == 0]

        # 5. initialize self.rankVec : pagerank vector -> initialize properly(uniformly)
        self.rankVec = [1.0 / self.N] * self.N
    ...
    def solveRankIter(self, oldRankVec):
        ...
        randomContrib = (1.0 - self.alpha) / self.N

        # Contributions from sinks
        sinkContrib = self.alpha * sum([oldRankVec[i] for i in self.sinks]) / self.N

        for x in self.nodeIDs:
            newRankVec[x] = randomContrib + sinkContrib

            # Contributions from inLinks
            for y in self.inList[x]:
                newRankVec[x] += self.alpha * oldRankVec[y] / self.outDegree[y]
    ...
    def solveRankToEps(self, eps):
        ...
        while delta >= eps:
            rankVec = list(newRankVec)  # copy
            newRankVec = self.solveRankIter(rankVec)
            delta = 0
            for i in xrange(self.N):
                delta = max(delta, abs(rankVec[i] - newRankVec[i]))
                if delta >= eps:
                    break
