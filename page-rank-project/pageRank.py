...
class PageRank(object):
    ...
    def __init__(self, inFileName, alpha=.85, selfLoops=False):
        ...
        self.outFileName = util.makeResOutFileName(
            self.inFileName, self.alpha, self.selfLoops)
...
def plotRes(prObj):
    ...
    plt.bar(index, pltPRVals75, bar_width,
            alpha=opacity,
            color='b',
            label='alpha=.75')

    plt.bar(index + bar_width, pltPRVals85, bar_width,
            alpha=opacity,
            color='r',
            label='alpha=.85')
    plt.bar(index + 2*bar_width, pltPRVals95, bar_width,
            alpha=opacity,
            color='g',
            label='alpha=.95')
    ...
    ttlStr = 'Scores for dataset {} for sites ranked {} through {}\n'.format(
        prObj.inFileName, (stSiteLoc + 1),
        endSiteLoc) + 'for 3 alpha values using {} sink handling'.format(
        'self-loop' if prObj.selfLoops else 'Type 3')
    plt.title(ttlStr)
    plt.legend()
...
def calcRes(prObj, args, prMadeTime):
    ...
    print(
        '\nPageRank vector calculated.  Elapsed time : {} seconds'.format(
            rvDoneTime -
            prMadeTime))
...
def main():
    ...
    parser.add_argument(
        '-i',
        '--infile',
        help='Input file adjacency information of graph',
        default='testCaseSmall.txt',
        dest='inFileName')
    parser.add_argument(
        '-a',
        '--alpha',
    ...
    parser.add_argument(
        '-e',
        '--epsilon',
        help='Epsilon Value for convergence test.',
        type=float,
        default=1e-10,
        dest='eps')
    parser.add_argument(
        '-p',
        '--plot',
        help='Plot pregenerated results instead of executing algorithm',
        action='store_true',
        default=False,
        dest='plot')
    parser.add_argument(
        '-s',
        '--selfloop',
        help='Use Self Loops to handle sinks.',
        action='store_true',
        default=False,
        dest='selfLoops')
    ...
    # input file name is used to build names for output files to save rank
    # vector and ranked ordering of nodes
    startTime = time.time()
    print('\nMaking pagerank object using input file {} and alpha {} and {} sink handling.'.format(
        args.inFileName, args.alpha, 'self-loop' if args.selfLoops else 'Type 3'))
    prObj = PageRank(
        args.inFileName,
        alpha=float(
            args.alpha),
        selfLoops=args.selfLoops)
    prMadeTime = time.time()
    print(
        '\nPageRank object made.  Elapsed time : {} seconds'.format(
            prMadeTime -
            startTime))
