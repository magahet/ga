...
# random.randint(a,b) : returns random integer in range [a,b) (inclusive
# beginning and exclusive ending values)
...
genHashes (boolean) : whether or not to generate new hash seeds/coefficients
        ...
        if genHashes:
            ...
            self.seeds = [random.randint(2**32) for _ in xrange(self.k)]
    ...
    def getHashList(self, x):
        ...
        # your code goes here
        hashes = [random.RandomState(s + x) for s in self.seeds]
        res = [h.randint(self.n) for h in hashes]
        return res
...
genHashes (boolean) : whether or not to generate new hash seeds/coefficients
        ...
        if genHashes:
            ...
            self.a = [random.randint(self.n) for _ in xrange(self.k)]
            self.b = [random.randint(self.n) for _ in xrange(self.k)]
    ...
    def getHashList(self, x):
        ...
        """
        res = [
            ((self.a[i] * x + self.b[i]) % self.P) % self.n for
            i in xrange(self.k)
        ]
...
Args:
    ...
    if type 1 hash :
        config['seeds'] (list of k ints) : seed values for k hash functions
                                           for type 1 hash function
    if type 2 hash :
        config['a'] (list of k ints) : a coefficients for k hash functions for
                                       type 2 hash function
        config['b'] (list of k ints) : b coefficients for k hash functions for
                                       type 2 hash function

    genHashes (boolean) : whether to generate new hash seeds/coefficients
                            (Task 2 provides hash seeds/coefficients,
                            Tasks 1 and 3 require you to make them yourself)
        """
        # task this boom filter is performing
        self.task = config['task']
        # task 1 and 3 reguire generated seeds for hashes, task 2 uses provided
        # seeds/coefficients
    ...
    def contains(self, x):
        """Indicates whether data structure contains x, using self.hashFunc,
        with the possibility of false positives
...
"""
function will take data list, insert first m elements into bloom filter,
and check all elements in datalist for membership,
returning a list of results of check
    Args :
        data (list) : list of integer data to add
        bf (object) : bloom filter object
        m (int) : number of elements to add to bloom filter from data
                  (first m elements)
    Returns :
...
function will support required test for Task 2.  DO NOT MODIFY.
    configData : dictionary of configuration data required to build and test
                 bloom filter
"""
...
def task2(configData):
    ...
    # load appropriate validation data list for this hash function and compare
    # to results
...
"""
these two functions are added for your convenience, if you choose to use this
code to perform tasks 1 and 3
"""
...
# NOTE task 3 will require you to remake your bloom filter multiple times to
# perform the appropriate trials
# this will necessitate either making a new bloom filter constructor or
# changing the config dictionary to
# hold the appropriate values for k and n (filter size) based on c value,
# derived as in the notes
# REMEMBER for type 2 hashes n must be prime.  util.findNextPrime(n) is
# provided for you to use to find the next largest
